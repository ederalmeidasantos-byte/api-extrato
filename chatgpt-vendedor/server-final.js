...
// Middleware personalizado para parsing JSON
app.use((req, res, next) => {
  if (req.method === 'POST' && req.headers['content-type']?.includes('application/json')) {
    ...
    req.on('end', () => {
      try {
        console.log('📝 [JSON] Body recebido:', body);
        req.body = JSON.parse(body);
        console.log('✅ [JSON] JSON parseado com sucesso:', req.body);
        next();
      } catch (error) {
        console.error('❌ [JSON] Erro ao parsear JSON:', error.message);
        console.error('❌ [JSON] Body que causou erro:', body);
...
console.log('🤖 ChatGPT Vendedor FINAL rodando na porta', PORT);
console.log('🔗 Webhook: http://localhost:3006/webhook/kentro');
console.log('🧪 Teste CPF: http://localhost:3006/teste-cpf');
