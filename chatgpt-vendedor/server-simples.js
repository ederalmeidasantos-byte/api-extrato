...
app.get('/', (req, res) => {
  res.json({
    sistema: 'ChatGPT Vendedor - Simples',
    status: 'online',
    porta: PORT,
...
// Webhook principal para Kentro - Busca completa de cliente e propostas
app.post('/webhook/kentro', async (req, res) => {
  try {
    ...
    // Buscar dados completos do cliente (sempre que possível)
    if (cpf) {
    ...
    if (clienteData && clienteData.success && clienteData.cliente) {
      ...
      // Resposta genérica se não tem dados do cliente
...
app.listen(PORT, () => {
  console.log(`🤖 ChatGPT Vendedor Simples rodando na porta ${PORT}`);
  console.log(`🔗 Webhook: http://localhost:${PORT}/webhook/kentro`);
  console.log(`🧪 Teste CPF: http://localhost:${PORT}/teste-cpf`);
});
