...
function calcularTrocoEndpoint(JSON_DIR) {
  return (_req, res) => {
    try {
      ...
      const simulador_link = `${baseUrl}/simulador?id=${fileId}`;
      
      // Gerar link do resumo do cliente baseado no NB (matrícula)
      const matricula = extrato?.beneficio?.nb || null;
      const cliente_resumo_link = matricula 
        ? `${baseUrl}/operacional/buscar-cliente.html?nb=${matricula}`
        : null;
      ...
      return res.json({
        ...
        cliente_resumo_link: cliente_resumo_link
      });
    } catch (err) {
