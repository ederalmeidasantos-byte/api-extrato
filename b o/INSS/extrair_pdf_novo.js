...
// ================== Pós-processamento ==================
function posProcessarJSON(parsed) {
  console.log("🔄 [UPLOAD] Aplicando pós-processamento...");

  // Processar contratos de empréstimo
  parsed.contratos = (parsed.contratos || []).map(contrato => {
    ...
    const parcelasPagas = valorPago > 0 ? Math.floor(valorPago / valorParcela) : 0;
    const prazoRestante = Math.max(0, qtdeParcelas - parcelasPagas);
...
export async function extrairDeUpload({ fileId, pdfPath, jsonDir, ttlMs, idoportunidade }) {
  try {
    ...
    console.log("🔄 [UPLOAD] Aplicando pós-processamento...");
    const finalJSON = posProcessarJSON(parsed);
    console.log("✅ [UPLOAD] Pós-processamento concluído");
