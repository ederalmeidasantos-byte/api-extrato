// Teste completo da formatação do troco
console.log("💰 TESTE COMPLETO DA FORMATAÇÃO DO TROCO");
...
function calcularTrocoParaTaxa(contrato, taxa) {
    ...
    const saldoDevedor = contrato.saldo_devedor || 17561.36; // Valor do teste
...
const contrato = {
    ...
    saldo_devedor: 17561.36
};

console.log("📊 Teste com contrato:");
console.log(`   Parcela: "${contrato.valor_parcela}"`);
console.log(`   Saldo Devedor: ${contrato.saldo_devedor}`);

// Teste com diferentes taxas
const taxas = [1.66, 1.79, 1.85];

console.log(`\n💰 Teste com diferentes taxas:`);
taxas.forEach(taxa => {
    const troco = calcularTrocoParaTaxa(contrato, taxa);
    console.log(`   Taxa ${taxa}%: R$ ${troco}`);
});

// Verificar se há problema com a formatação
console.log(`\n🔍 Verificação da formatação:`);
const troco = 4366.42;
console.log(`   Valor: ${troco}`);
console.log(`   formatBRNumber: "${formatBRNumber(troco)}"`);
console.log(`   toLocaleString pt-BR: "${Number(troco).toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}"`);
console.log(`   toLocaleString en-US: "${Number(troco).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}"`);
