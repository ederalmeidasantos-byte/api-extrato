...
class ClientManager {
    ...
    async createOrUpdateClient(clientData) {
        ...
        try {
            ...
            if (result.success) {
                // Usar o ID retornado pelo servidor (pode ser diferente se já existia)
                clientId = result.clientId;
                client.id = clientId;
                console.log(`✅ Cliente salvo no servidor com ID: ${clientId}`);
            } else {
                console.error('❌ Erro ao salvar cliente no servidor:', result.error);
    ...
    async loadFromServer() {
        try {
            ...
            if (!syncResult.success) {
                console.warn('⚠️ Falha ao carregar do servidor, criando dados de teste...');
                this.createTestData();
            } else {
