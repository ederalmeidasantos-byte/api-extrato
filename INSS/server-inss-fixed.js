...
const HTTPS_PORT = 443; // Porta HTTPS
...
app.get('/api/calcular/:fileId', async (req, res) => {
  try {
    ...
    const calcularTroco = calcularTrocoEndpoint(
      path.join('/app', 'var', 'data', 'extratos')
    );
...
app.get('/extrato/:fileId', async (req, res) => {
  try {
    ...
    const jsonPath = path.join('/app', 'var', 'data', 'extratos', `extrato_${fileId}.json`);
    const pdfPath = path.join('/app', 'var', 'data', 'extratos', `extrato_${fileId}.pdf`);
    ...
    if (!fs.existsSync(pdfPath)) {
      ...
      try {
        ...
        console.log('📥 [INSS] Baixando PDF via proxy:', proxyUrl);
        
        const response = await fetch(proxyUrl, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/x-www-form-urlencoded',
            'User-Agent': 'INSS Server'
          },
          body: formData
        });
        ...
        console.log('📄 [INSS] PDF Header:', pdfHeader);
        
        if (!pdfHeader.startsWith('%PDF-')) {
          console.error('❌ [INSS] PDF inválido!');
          throw new Error('PDF inválido recebido da Kentro');
        }
        
        console.log('📥 [INSS] PDF baixado via proxy - Tamanho:', pdfData.length, 'bytes');
        
        // Salvar PDF
        await fsp.writeFile(pdfPath, pdfData);
        console.log('✅ PDF salvo em', pdfPath, '- Tamanho:', pdfData.length, 'bytes');
    ...
    const resultado = await extrairDeUpload({
      fileId: fileId,
      pdfPath: pdfPath,
      jsonDir: path.join('/app', 'var', 'data', 'extratos'),
      ttlMs: 14 * 24 * 60 * 60 * 1000, // 14 dias
      idoportunidade: idoportunidade
    });
...
app.post('/extrair', async (req, res) => {
  try {
    console.log('📄 [INSS] Requisição recebida:', req.body);
    console.log('📄 [INSS] Content-Type:', req.get('Content-Type'));
    console.log('📄 [INSS] Headers:', req.headers);
    ...
    console.error('❌ [INSS] Erro ao extrair dados:', error);
    console.error('❌ [INSS] Tipo do erro:', error.constructor.name);
    console.error('❌ [INSS] Mensagem:', error.message);
    console.error('❌ [INSS] Stack:', error.stack);
...
httpsServer.listen(HTTPS_PORT, '0.0.0.0', () => {
  console.log(`🔒 [INSS] Servidor HTTPS rodando na porta ${HTTPS_PORT}`);
  console.log(`🌐 [INSS] Acesse: https://inss.lunasdigital.com.br/inss/simulador.html`);
});
